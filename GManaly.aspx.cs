...
public partial class GManaly : System.Web.UI.Page
{
    ...
    private void UpdateGridViews()
    {
        ...
        string selectCommand = "SELECT * FROM [dbo].[summary_combined_scores_" + term + "_" + year + "]";
    ...
    private void LoadChartData(DataTable dt)
    {
        ...
        chartArea.AxisX.MajorGrid.Enabled = false; // Removes grid lines for clarity
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            for (int i = 1; i < dt.Columns.Count - 1; i += 2)
            {
                ...
                Series weekSeries = new Series(weekName)
                {
                    ...
                    BorderWidth = 2,
                    IsValueShownAsLabel = false // Disable value labels on the chart
                };
            ...
            // Configure the data table settings
            foreach (Series series in ScoresChart.Series)
            {
                series.SmartLabelStyle.Enabled = false; // Avoid overlap by disabling smart labels if not needed
                series.LabelFormat = "F2"; // Format the values appropriately, e.g., two decimal places
            }

            // Set up data table
            DataTable dataTable = dt.DefaultView.ToTable(true, "school_name", dt.Columns[1].ColumnName, "Moving_Avg"); // Filter columns
            ScoresChart.DataBindTable(dataTable.DefaultView, "school_name");

            // Configure the legend
            Legend legend = new Legend("Legend")
            {
                Title = "Overall Week Performance & Moving Avg",
                Docking = Docking.Bottom
            };
            ScoresChart.Legends.Add(legend);

            // Position the chart properly
            chartArea.Position = new ElementPosition(0, 0, 100, 70); // Adjust to fit data table below
